...
if ( $title ) {
	$output .= '<header class="card__header">';
		...
		if ( $show_full_table_link ) {
			$output .= '<a href="' . get_permalink( $id ) . '" class="btn btn-default btn-outline btn-xs card-header__button">' . esc_html__( 'View full table', 'sportspress' ) . '</a>';
		}
	$output .= '</header>';
}
...
$output .= '<table class="table table-hover table-standings sp-league-table sp-data-table' . ( $sortable ? ' sp-sortable-table' : '' ) . ( $responsive ? ' sp-responsive-table ' . $identifier : '' ). ( $scrollable ? ' sp-scrollable-table' : '' ) . ( $paginated ? ' sp-paginated-table' : '' ) . '" data-sp-rows="' . $rows . '">' . '<thead>' . '<tr>';
...
$output .= '<th class="data-rank">' . esc_html__( 'Pos', 'sportspress' ) . '</th>';
