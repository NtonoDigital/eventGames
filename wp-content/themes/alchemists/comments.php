...
if ( ! alchemists_sp_preset('football') ) {
	$post_commments_classes[] = 'card--lg';
}
...
<div id="comments" class="<?php echo esc_attr( implode(' ', $post_commments_classes) ); ?>">
	...
	<header class="post-commments__header card__header">
		<h4><?php esc_html_e( 'Comments', 'alchemists' ); ?> (<?php echo esc_html( get_comments_number() ); ?>)</h4>
...
$comment_form_classes = array('post-comment-form', 'card');
...
if ( ! alchemists_sp_preset('football') ) {
	$comment_form_classes[] = 'card--lg';
	$form_submit_classes[] = 'btn-default';
} else {
	$form_submit_classes[] = 'btn-primary-inverse';
}
?>
