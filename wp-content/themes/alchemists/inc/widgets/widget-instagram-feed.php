...
class Alchemists_Widget_Instagram_Feed extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => esc_html__( 'Display Instagram feed as a widget.', 'alchemists' ),
		);
		$control_ops = array(
	...
	function widget( $args, $instance ) {
		...
		$btn_label    = isset( $instance['btn_label'] ) ? $instance['btn_label'] : esc_html__( 'Follow Our Instagram', 'alchemists' );
	...
	function form( $instance ) {
		...
		$defaults = array(
			...
			'layout_style' => '3cols',
			'show_button'  => 'on',
			'btn_label'    => esc_html__( 'Follow Our Instagram', 'alchemists' ),
		);
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'user_name' ) ); ?>"><?php esc_html_e( 'Username:', 'alchemists' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'user_id' ) ); ?>"><?php esc_html_e( 'User ID:', 'alchemists' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'access_token' ) ); ?>"><?php esc_html_e( 'Access Token:', 'alchemists' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'img_count' ) ); ?>"><?php esc_html_e( 'Number of images:', 'alchemists' ); ?></label>
		...
		<p>
			...
			<select id="<?php echo esc_attr( $this->get_field_id( 'layout_style' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'layout_style' ) ); ?>" class="widefat">
				<option value="3cols" <?php echo ( '3cols' == $instance['layout_style'] ) ? 'selected="selected"' : ''; ?>><?php esc_html_e( '3 Columns', 'alchemists' ); ?></option>
				<option value="4cols" <?php echo ( '4cols' == $instance['layout_style'] ) ? 'selected="selected"' : ''; ?>><?php esc_html_e( '4 Columns', 'alchemists' ); ?></option>
			</select>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_button' ) ); ?>"><?php esc_attr_e( 'Show Follow Button', 'alchemists' ); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'btn_label' ) ); ?>"><?php esc_html_e( 'Follow Button Label:', 'alchemists' ); ?></label>
