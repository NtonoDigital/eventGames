...
if ( $title ) {
	$output .= '<header class="card__header">';
		...
		if ( $show_all_staff_link ) {
			$output .= '<a class="sp-staff-directory-link sp-view-all-link btn btn-default btn-outline btn-xs card-header__button" href="' . get_permalink( $id ) . '">' . esc_html__( 'View all staff', 'sportspress' ) . '</a>';
		}
	$output .= '</header>';
}
...
$output .= '<div class="table-responsive sp-table-wrapper">' .
	'<table class="table table--lg sp-staff-directory sp-data-table' . ( $sortable ? ' sp-sortable-table' : '' ) . ( $scrollable ? ' sp-scrollable-table' : '' ) . ( $paginated ? ' sp-paginated-table' : '' ) . '" data-sp-rows="' . $rows . '">' . '<thead>' . '<tr>';
