...
$i = 0;
?>
<div class="card card--has-table sp-template sp-template-event-performance sp-template-event-performance-<?php echo esc_attr( $mode ); ?><?php if ( isset( $class ) ) { echo ' ' . $class; } ?>">
	<?php if ( $caption ): ?>
		<header class="card__header">
			<h4 class="sp-table-caption"><?php echo esc_html( $caption ); ?></h4>
		</header>
	<?php endif; ?>
	<div class="card__content">
		<div class="sp-table-wrapper">
			<div class="table-responsive">
				<table class="table sp-event-performance sp-data-table<?php if ( $mode == 'values' ) { ?><?php if ( $scrollable ) { ?> sp-scrollable-table<?php }if ( $responsive ) { echo ' sp-responsive-table '.$identifier; } if ( $sortable ) { ?> sp-sortable-table<?php } ?><?php } ?>">
					<thead>
						<tr>
							<?php if ( $mode == 'values' ): ?>
								<?php if ( $show_players ): ?>
									...
									<th class="data-name">
										...
										<?php } else { ?>
											<?php esc_html_e( 'Player', 'sportspress' ); ?>
										<?php } ?>
									</th>
								<?php endif; ?>
							...
							foreach ( $data as $player_id => $row ):
								...
								if ( $stars_type ):
									...
									if ( $player_stars ):
										switch ( $stars_type ):
											case 1:
												$name .= ' <span class="sp-event-stars"><i class="sp-event-star dashicons dashicons-star-filled" title="' . esc_html__( 'Player of the Match', 'sportspress' ) . '"></i></span>';
												break;
											case 2:
												$name .= ' <span class="sp-event-stars">' . str_repeat( '<i class="sp-event-star dashicons dashicons-star-filled" title="' . esc_html__( 'Stars', 'sportspress' ) . '"></i>', $player_stars ) . '</span>';
												break;
											case 3:
												$name .= ' <span class="sp-event-stars"><i class="sp-event-star sp-event-star-' . $player_stars . '  dashicons dashicons-star-filled" title="' . esc_html__( 'Stars', 'sportspress' ) . '"></i><span class="sp-event-star-number">' . $player_stars . '</span></span>';
												break;
										endswitch;
									endif;
								endif;
								...
								echo '<td class="data-name" data-label="' . ( isset( $section_label ) ? $section_label : esc_html__( 'Player', 'sportspress' ) ) .'">' . $name . '</td>';
							...
							if ( $show_total && ( ! $primary || sizeof( array_intersect_key( $totals, array_flip( (array) $primary ) ) ) ) ) {
								...
								<tr class="sp-total-row <?php echo ( $i % 2 == 0 ? 'odd' : 'even' ); ?>">
									...
									if ( $show_players ):
										...
										if ( $mode == 'values' ):
											echo '<td class="data-name" data-label="&nbsp;">' . esc_html__( 'Total', 'sportspress' ) . '</td>';
										endif;
									endif;
