...
if ( 1 == $sections ) {
	$section_order = array( 1 => esc_html__( 'Defense', 'sportspress' ), 0 => esc_html__( 'Offense', 'sportspress' ) );
} elseif ( 0 == $sections ) {
	$section_order = array( esc_html__( 'Offense', 'sportspress' ), esc_html__( 'Defense', 'sportspress' ) );
} else {
	...
	foreach ( $section_order as $section_id => $section_label ) {
		...
		if ( $show_career_totals ) {
			sp_get_template( 'player-statistics-league.php', array(
				...
				'caption' => esc_html__( 'Career Total', 'sportspress' ),
				'scrollable' => $scrollable,
				'hide_teams' => true,
			) );
		}
