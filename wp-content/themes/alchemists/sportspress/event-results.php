...
if ( ! isset( $caption ) ) $caption = esc_html__( 'Results', 'sportspress' );
...
if ( empty( $table_rows ) ):
	...
	$output .= '<header class="card__header"><h4 class="sp-table-caption">' . $caption . '</h4></header>';

	$output .= '<div class="card__content">';
	$output .= '<div class="sp-table-wrapper">' . '<div class="table-responsive">' .
		'<table class="table sp-event-results sp-data-table' . ( $scrollable ? ' sp-scrollable-table' : '' ) . '"><thead>' . '<tr>' .
		'<th class="data-name">' . esc_html__( 'Team', 'sportspress' ) . '</th>';
	foreach( $labels as $key => $label ):
...
?>
<div class="card card--has-table sp-template sp-template-event-results">
