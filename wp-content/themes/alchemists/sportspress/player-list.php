	...
	if ( $title ) {
		$output .= '<header class="card__header">';
			...
			if ( $show_all_players_link ):
				$output .= '<a href="' . get_permalink( $id ) . '" class="btn btn-default btn-outline btn-xs card-header__button">' . esc_html__( 'View all players', 'sportspress' ) . '</a>';
			endif;
...
foreach ( $groups as $group ):
	...
	if ( $grouping === 'position' ) {
		$output .= '<div class="card card--has-table sp-template sp-template-player-list">';
	} else {
		$output .= '<div class="card__content sp-template sp-template-player-list">';
	}
	...
	$output .= '<div class="table-responsive sp-table-wrapper">' .
		'<table class="table table--lg team-roster-table sp-player-list sp-data-table' . ( $sortable ? ' sp-sortable-table' : '' ). ( $responsive ? ' sp-responsive-table ' . $identifier : '' ) . ( $scrollable ? ' sp-scrollable-table' : '' ) . ( $paginated ? ' sp-paginated-table' : '' ) . '" data-sp-rows="' . $rows . '">';
