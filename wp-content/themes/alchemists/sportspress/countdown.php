					...
					<div class="match-preview__vs">
						...
						<div class="match-preview__match-info">
							<?php if ( isset( $show_date ) && $show_date ) : ?>
								<time class="match-preview__match-time" datetime="<?php echo esc_attr( $post->post_date ); ?>"><?php echo mysql2date( get_option( 'time_format' ), $post->post_date ); ?></time>
							<?php endif; ?>
							<?php
			...
			<?php endif; ?>
				...
				<div class="countdown__content">
					<div class="countdown sp-countdown<?php if ( $days >= 10 ): ?> long-countdown<?php endif; ?>">
						<time class="countdown-counter" datetime="<?php echo esc_attr( $post->post_date ); ?>"<?php if ( $live ): ?> data-countdown="<?php echo str_replace( '-', '/', get_gmt_from_date( $post->post_date ) ); ?>"<?php endif; ?>>
			...
			<?php if ( alchemists_sp_preset( 'soccer' ) ) : ?>
				<?php if ( $link_events ) : ?>
					<div class="match-preview__action match-preview__action--ticket">
						<a href="<?php echo get_post_permalink( $post->ID, false, true ); ?>" class="btn btn-primary-inverse btn-lg btn-block"><?php echo esc_html_x( 'Read More', 'event countdown', 'alchemists' ); ?></a>
					</div>
				<?php endif; ?>
			<?php endif; ?>
