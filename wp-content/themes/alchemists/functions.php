...
if( !function_exists( 'alchemists_scripts' ) ) {
	function alchemists_scripts() {
		...
		// Main CSS
		wp_enqueue_style( 'alchemists-style', get_template_directory_uri() . '/assets/css/style-' . $sport . '.css', array(), THEME_VERSION );
		...
		if( alchemists_wc_exists() ) {
			wp_enqueue_style( 'woocommerce', get_template_directory_uri() . '/assets/css/woocommerce-' . $sport . '.css', array(), false);
		}

		// Add styles if Sporspress installed
		if ( class_exists( 'SportsPress' ) ) {
			wp_enqueue_style( 'alchemists-sportspress', get_template_directory_uri() . '/assets/css/sportspress-' . $sport . '.css', array(), THEME_VERSION);
		}
		...
		wp_register_script( 'alchemists-marquee', get_template_directory_uri() . '/assets/vendor/marquee/jquery.marquee.min.js', array(), '1.5.0', true );
...
if ( is_child_theme() ) {
	function alchemists_load_child_theme_styles() {
		wp_enqueue_style( 'alchemists-child', get_stylesheet_uri(), array(), THEME_VERSION );
	}
...
if(!function_exists('alchemists_wp_site_icon')) {
	function alchemists_wp_site_icon() {
		if ( ! ( function_exists( 'has_site_icon' ) && has_site_icon() ) ) {
			...
			$sport_favicon_uri = esc_url( get_template_directory_uri() ) . '/assets/images/favicons/';

			if ( alchemists_sp_preset('soccer') ) {
				$sport_favicon_uri = esc_url( get_template_directory_uri() ) . '/assets/images/soccer/favicons/';
			} elseif ( alchemists_sp_preset('football') ) {
				$sport_favicon_uri = esc_url( get_template_directory_uri() ) . '/assets/images/football/favicons/';
			}

			echo '<link rel="shortcut icon" href="' . $sport_favicon_uri . 'favicon.ico" type="image/x-icon" />' . "\n";
			echo '<link rel="apple-touch-icon" sizes="120x120" href="' . $sport_favicon_uri . 'favicon-120.png">' . "\n";
			echo '<link rel="apple-touch-icon" sizes="152x152" href="' . $sport_favicon_uri . 'favicon-152.png">' . "\n";
